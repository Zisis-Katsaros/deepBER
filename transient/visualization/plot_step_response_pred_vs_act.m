function plot_step_response_pred_vs_act(t, V_in, V_out_main_pred, V_out_main_act, V_out_next1_pred, V_out_next1_act, V_out_fext1_pred, ...
    ...
    % Vin
    h_in = plot(ax, t*1e9, V_in, 'k--', 'LineWidth', 1.5);
    plot_handles = [plot_handles, h_in];
    legend_labels = [legend_labels, {'V_{in} (Source)'}];

    % Vout_main
    h_main_act = plot(ax, t*1e9, V_out_main_act, '-', 'Color', [0, 0, 0.8], 'LineWidth', 1.5);
    
    if ~isempty(V_out_main_pred_adj)
        h_main_pred = plot(ax, t*1e9, V_out_main_pred, ':', 'Color', [0.4, 0.4, 1.0], 'LineWidth', 1.5); % Lighter dotted for raw
        h_main_pred_adj = plot(ax, t*1e9, V_out_main_pred_adj, '--', 'Color', [0, 0.5, 0], 'LineWidth', 1.5); % Distinct Green for adjusted
        ...
        h_main_pred = plot(ax, t*1e9, V_out_main_pred, '--', 'Color', [0, 0, 0.8], 'LineWidth', 1.2);
    ...
    if ~isempty(V_out_target)
        h_target = yline(ax, V_out_target, 'm-.', 'LineWidth', 1.5);
    ...
    if ~isempty(V_out_next1_act) && ~isempty(V_out_next1_pred)
        h_n1_act = plot(ax, t*1e9, V_out_next1_act, '-', 'Color', [0, 0.8, 0.8], 'LineWidth', 1.2);
        h_n1_pred = plot(ax, t*1e9, V_out_next1_pred, '--', 'Color', [0, 0.8, 0.8], 'LineWidth', 1.0);
    ...
    if ~isempty(V_out_fext1_act) && ~isempty(V_out_fext1_pred)
        h_f1_act = plot(ax, t*1e9, V_out_fext1_act, '-', 'Color', [0.8, 0, 0.4], 'LineWidth', 1.2);
        h_f1_pred = plot(ax, t*1e9, V_out_fext1_pred, '--', 'Color', [0.8, 0, 0.4], 'LineWidth', 1.0);
    ...
    if ~isempty(V_out_next2_act) && ~isempty(V_out_next2_pred)
        h_n2_act = plot(ax, t*1e9, V_out_next2_act, '-', 'Color', [0, 0.8, 0.4], 'LineWidth', 1.2);
        h_n2_pred = plot(ax, t*1e9, V_out_next2_pred, '--', 'Color', [0, 0.8, 0.4], 'LineWidth', 1.0);
    ...
    if ~isempty(V_out_fext2_act) && ~isempty(V_out_fext2_pred)
        h_f2_act = plot(ax, t*1e9, V_out_fext2_act, '-', 'Color', [0.8, 0, 0.8], 'LineWidth', 1.2);
        h_f2_pred = plot(ax, t*1e9, V_out_fext2_pred, '--', 'Color', [0.8, 0, 0.8], 'LineWidth', 1.0);
    ...
    xlabel(ax, 'Time (ns)');
    ylabel(ax, 'Voltage (V)');
    legend(ax, plot_handles, legend_labels, 'Location', 'best');
    hold(ax, 'off');
end